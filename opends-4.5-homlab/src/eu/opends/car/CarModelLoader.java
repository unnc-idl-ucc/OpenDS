...
public class CarModelLoader
{
	...
	public float custom_scale = 1.0f;
	
	
	@SuppressWarnings("static-access")
	public CarModelLoader(Simulator sim, Car car, String modelPath, float mass)
	{	
		if(car instanceof TrafficCar) this.custom_scale = 1.0f;
		else this.custom_scale = 1.0f;
		
		
		//Shi Cao
		//for .j3o cars, scale needs to be 1.0		
		//for .scene cars, scale needs to be larger than 1.0
		if (modelPath.contains(".scene")) custom_scale *= 1.5;
		...
		chassisScale.multLocal(this.custom_scale);
		...
		egoCamPos.y -= 0.5; // Shi Cao, try lowing the ego camera to better match driving simulator setup.
		...
		if(wheelScaleString != null)
			wheelScale = Float.parseFloat(wheelScaleString) * this.custom_scale;
		else
        ...
        if (shadowModelActive){
        	if (car instanceof SteeringCar){
        		try {
		    	    ...
		    	    NewChassisScale.multLocal(this.custom_scale);
		    	
		       	        // scale chassis 
		    	    highPolyChassis.scale(Float.parseFloat(propertiesChassis.getProperty("chassisScale.x"))*this.custom_scale, Float.parseFloat(propertiesChassis.getProperty("chassisScale.y"))*this.custom_scale, Float.parseFloat(propertiesChassis.getProperty("chassisScale.z"))*this.custom_scale);
		            	// rotate chassis
		            ...
		            String modelPathSteeringWheel = sim.getDrivingTask().getSceneLoader().getSteeringWheel();           
		       	    ...
		       	    newSteeringWheelScale.multLocal(this.custom_scale);
		       	    
		       	    	//scale steering wheel
		       	    highPolySteering.scale(Float.parseFloat(propertiesSteeringWheel.getProperty("steeringWheelScale.x"))*this.custom_scale, Float.parseFloat(propertiesSteeringWheel.getProperty("steeringWheelScale.y"))*this.custom_scale, Float.parseFloat(propertiesSteeringWheel.getProperty("steeringWheelScale.z"))*this.custom_scale);
				    ...
				    String propertiesPathInstrumentCluster = modelPathInstrumentCluster.replace(".scene", ".properties");
		       	    ...
		       	    instrumnetCluster.scale(Float.parseFloat(propertiesInstrumentCluster.getProperty("InstrumentClusterScale.x"))*this.custom_scale, Float.parseFloat(propertiesInstrumentCluster.getProperty("InstrumentClusterScale.y"))*this.custom_scale, Float.parseFloat(propertiesInstrumentCluster.getProperty("InstrumentClusterScale.z"))*this.custom_scale);
		       	    // rotate steering wheel
        ...
        */
        	...
        	if (shadowModelActive){
        		try {
	    	        ...
	    	        String modelPathWheelFR = sim.getDrivingTask().getSceneLoader().getWheelFrontRight();           
		       	    ...
		       	    highPolyWheelFrontRight.scale(Float.parseFloat(propertiesWheelFR.getProperty("WheelFRScale.x"))*this.custom_scale, Float.parseFloat(propertiesWheelFR.getProperty("WheelFRScale.y"))*this.custom_scale, Float.parseFloat(propertiesWheelFR.getProperty("WheelFRScale.z"))*this.custom_scale);
		        	...
		        	geom_wheel_fr_v2.center();
		 	        ...
		 	        String modelPathWheelFL = sim.getDrivingTask().getSceneLoader().getWheelFrontLeft();           
		       	    ...
		       	    highPolyWheelFrontLeft.scale(Float.parseFloat(propertiesWheelFL.getProperty("WheelFLScale.x"))*this.custom_scale, Float.parseFloat(propertiesWheelFL.getProperty("WheelFLScale.y"))*this.custom_scale, Float.parseFloat(propertiesWheelFL.getProperty("WheelFLScale.z"))*this.custom_scale);
			        ...
			        node_wheel_br.attachChild(highPolyWheelBackRight);
		 	        String modelPathWheelBR = sim.getDrivingTask().getSceneLoader().getWheelBackRight();           
		       	    ...
		       	    highPolyWheelBackRight.scale(Float.parseFloat(propertiesWheelBR.getProperty("WheelBRScale.x"))*this.custom_scale, Float.parseFloat(propertiesWheelBR.getProperty("WheelBRScale.y"))*this.custom_scale, Float.parseFloat(propertiesWheelBR.getProperty("WheelBRScale.z"))*this.custom_scale);
			        ...
			        node_wheel_bl.attachChild(highPolyWheelBackLeft);
		 	        String modelPathWheelBL = sim.getDrivingTask().getSceneLoader().getWheelBackLeft();           
		       	    ...
		       	    highPolyWheelBackLeft.scale(Float.parseFloat(propertiesWheelBL.getProperty("WheelBLScale.x"))*this.custom_scale, Float.parseFloat(propertiesWheelBL.getProperty("WheelBLScale.y"))*this.custom_scale, Float.parseFloat(propertiesWheelBL.getProperty("WheelBLScale.z"))*this.custom_scale);
		...
		if(car instanceof SteeringCar)
		{
			...
			if(interiorPath != null)
			{
				...
				interiorScale.multLocal(this.custom_scale);
