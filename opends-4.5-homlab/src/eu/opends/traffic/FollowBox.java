...
public class FollowBox 
{
	...
	{
		this.sim = sim;
		this.setTrafficObject(trafficObject);
...
//		System.out.println(this.motionControl.getTraveledDistance());
		
//		System.out.println(this.trafficObject.getName() );  //when paused, here won't show drivingCar
		
		if( sim.isPause()     )   //maxDistanceExceeded(trafficObjectPos) ||
		...
		if(sim.isPause() || counter<20)  // Cao, 2018-03-12, changed from if(sim.isPause() || counter<3)  
		{
			motionControl.setSpeed(0f);
			counter++;
		}
	...
	private void performWayPointChange(int index)
	{
		...
		getTrafficObject().setPosition(position);
		...
		getTrafficObject().setRotation(quaternion);
	}
	...
	public float getReducedSpeed()
	{
		...
		if(nextWP != null)
		{
			...
			if(targetSpeed < currentSpeed)
			{
				...
				float deceleration50Percent = 50f * getTrafficObject().getMaxBrakeForce()/getTrafficObject().getMass();
	...
	public TrafficObject getTrafficObject() {
		return trafficObject;
	}


	public void setTrafficObject(TrafficObject trafficObject) {
